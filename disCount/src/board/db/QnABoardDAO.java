...
public class QnABoardDAO {
	...
	// 글 등록
		public boolean boardInsert(BoardBean board) {
			...
			try {
				...
				pstmt = con.prepareStatement("select max(boardid) from qna_board_table");
				...
				sql = "insert into qna_board_table(board_member_id,boardtype,boardid,board_member_name,";
				sql += "board_title,board_context,writedate,readcount,board_file)"
						+ " values(?,?,QNA_BOARD_AUTOKEY.NEXTVAL,?,?,?,sysdate,?,?)";
	...
	public boolean boardDelete(int num) {
		String board_delete_sql = "delete from qna_board_table where boardid=?";
		int result = 0;
	...
	public boolean boardModify(BoardBean modifyboard) throws Exception {
		String sql = "update qna_board_table set board_title=?,";
	...
	public List getBoardList(int startid) {
		String board_list_sql = "SELECT * FROM (SELECT * FROM qna_board_table where boardid<=? order by boardid desc) WHERE ROWNUM<=10";
	...
	public boolean isBoardWriter(String num, String id) {
		...
		String board_sql = "select * from qna_board_table where boardid=?";
	...
	public BoardBean getDetail(int num) throws Exception {
		...
		try {
			con = ds.getConnection();
			pstmt = con.prepareStatement("select * from qna_board_table where boardid =?");
			pstmt.setInt(1, num);
			rs = pstmt.executeQuery();
	...
	public int getlastwrite(){
		...
		try {
			con = ds.getConnection();
			pstmt = con.prepareStatement("select boardid from qna_board_table order by boardid desc");
			rs = pstmt.executeQuery();
	...
	public int getListCount() {
		...
		try {
			con = ds.getConnection();
			pstmt = con.prepareStatement("select count(*) from qna_board_table");
			rs = pstmt.executeQuery();
	...
	public boolean addreadCount(int boardid) {
		...
		try {
			con = ds.getConnection();
			pstmt = con.prepareStatement("update qna_board_table SET readcount=(SELECT readcount from qna_board_table where boardid=?)+1 where boardid=?");
...
//이미지 업로드
	public boolean setimagesrc(String board_file,int boardid) {
		try {
			con = ds.getConnection();
			pstmt = con.prepareStatement("update qna_board_table SET board_file=? where boardid=?");
...
//이미지 불러오기
	public String getimagesrc(int boardid) {
		...
		try {
			con = ds.getConnection();
			pstmt = con.prepareStatement("select board_file from qna_board_table where boardid=?");
			pstmt.setInt(1, boardid);
			rs=pstmt.executeQuery();
			rs.next();
	...
	public List getSearchNameList(int startid,String searchname) {
		String board_list_sql = "SELECT * FROM (SELECT * FROM qna_board_table where boardid<=? and board_member_name like ? order by boardid desc) WHERE ROWNUM<=10";
		List list = new ArrayList();
		System.out.println(startid+"시작 id이름");
	...
	public List getSearchTitleList(int startid,String searchtitle) {
		String board_list_sql = "SELECT * FROM (SELECT * FROM qna_board_table where boardid<=? and board_title like ? order by boardid desc) WHERE ROWNUM<=10";
		List list = new ArrayList();
		System.out.println(startid+"시작 id이름");
