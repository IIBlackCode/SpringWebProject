...
public class BoardDAO {
	...
	// 글 등록
		public boolean boardInsert(BoardBean board) {
			...
			try {
				...
				pstmt = con.prepareStatement("select max(boardid) from main_board_table");
				...
				sql = "insert into main_board_table(board_member_id,boardtype,boardid,board_member_name,";
				sql += "board_title,board_context,writedate,readcount,board_file)"
						+ " values(?,?,MAIN_BOARD_AUTOKEY.NEXTVAL,?,?,?,sysdate,?,?)";
	...
	public boolean boardDelete(int num) {
		String board_delete_sql = "delete from main_board_table where boardid=?";
		int result = 0;
	...
	public boolean boardModify(BoardBean modifyboard) throws Exception {
		String sql = "update main_board_table set board_title=?,";
	...
	public List getBoardList(int startid) {
		String board_list_sql = "SELECT * FROM (SELECT * FROM main_board_table where boardid<=? order by boardid desc) WHERE ROWNUM<=10";
	...
	public boolean isBoardWriter(String num, String id) {
		...
		String board_sql = "select * from main_board_table where boardid=?";
	...
	public BoardBean getDetail(int num) throws Exception {
		...
		try {
			con = ds.getConnection();
			pstmt = con.prepareStatement("select * from main_board_table where boardid =?");
			pstmt.setInt(1, num);
			rs = pstmt.executeQuery();
	...
	public int getlastwrite(){
		...
		try {
			con = ds.getConnection();
			pstmt = con.prepareStatement("select boardid from main_board_table order by boardid desc");
			rs = pstmt.executeQuery();
	...
	public int getListCount() {
		...
		try {
			con = ds.getConnection();
			pstmt = con.prepareStatement("select count(*) from main_board_table");
			rs = pstmt.executeQuery();
	...
	public boolean addreadCount(int boardid) {
		...
		try {
			con = ds.getConnection();
			pstmt = con.prepareStatement("update main_board_table SET readcount=(SELECT readcount from MAIN_BOARD_TABLE where boardid=?)+1 where boardid=?");
...
//이미지 업로드
	public boolean setimagesrc(String board_file,int boardid) {
		try {
			con = ds.getConnection();
			pstmt = con.prepareStatement("update main_board_table SET board_file=? where boardid=?");
...
//이미지 불러오기
	public String getimagesrc(int boardid) {
		...
		try {
			con = ds.getConnection();
			pstmt = con.prepareStatement("select board_file from main_board_table where boardid=?");
			pstmt.setInt(1, boardid);
			rs=pstmt.executeQuery();
			rs.next();
	...
	public List getSearchNameList(int startid,String searchname) {
		String board_list_sql = "SELECT * FROM (SELECT * FROM main_board_table where boardid<=? and board_member_name like ? order by boardid desc) WHERE ROWNUM<=10";
		List list = new ArrayList();
		System.out.println(startid+"시작 id이름");
	...
	public List getSearchTitleList(int startid,String searchtitle) {
		String board_list_sql = "SELECT * FROM (SELECT * FROM main_board_table where boardid<=? and board_title like ? order by boardid desc) WHERE ROWNUM<=10";
		List list = new ArrayList();
		System.out.println(startid+"시작 id이름");
	...
	public List getHotboard()
	{
		...
		try {
			...
			pstmt = con.prepareStatement(
					...
					+"select * from qna_board_table where TO_CHAR(WRITEDATE, 'YYYYMMDD') = TO_CHAR(sysdate, 'YYYYMMDD')) where ROWNUM<=5 order by readcount desc"
					);
			rs = pstmt.executeQuery();
			...
			System.out.println("getHotboard 에러 : " + ex);
		} finally {
			if (rs != null)
				try {
					rs.close();
	...
	public List getnewMain()
	{
		...
		try {
			...
			System.out.println("getnewMain 에러 : " + ex);
		} finally {
			if (rs != null)
				try {
					rs.close();
	...
	//질문게시판 최근글 5개
	public List getnewQna()
	{
		List list = new ArrayList();
		try {
			con = ds.getConnection();
			pstmt = con.prepareStatement(
					"select * from (select * from qna_board_table order by WriteDate desc) where ROWNUM<=5"
					);
			rs = pstmt.executeQuery();
