...
<script type="text/javascript">
	function main_write() {
		location.href = "FreeBoardWrite.fr";
	}
	function board_search_list(){
		...
		location.href = "FreeBoardSearch.fr?board_search_type="+board_type+"&board_search_content="+board_content;
	}
</script>
	...
	<form action="" id="main_board_view" name="main_board_view"
		...
		<table class=table1  width="100%" cellpadding="0" cellspacing="0" border="0"  >
			...
			<thead>
            	...
            	<font size=5 , style="font-family:fantasy;">자유게시판</font>
            	 </td>
       	 		</tr>
    		</thead>
			...
			<tr onmouseover="this.style.background='#C4FDFF'" onmouseout="this.style.background='white'" style="background: url repeat-x; text-align: center;">
				...
				<td width="379"  style="text-align: left"><a
					href="FreeBoardDetailAction.fr?main_page_id=<%=boardbean.getBoardid()%>">
						<%=boardbean.getTitle()%>
				</a></td>
			  ...
			  -->
				...
				<%if(request.getParameter("board_search_type")==null){ %>
						<%
							if (!(boardsize < 10)) {
							%>
						<a href="FreeBoardAction.fr?main_list_end_boardid=<%=startnum - 1%>">Prev</a>
						<%
							}
						%>
						&nbsp;&nbsp;
						<a href="FreeBoardAction.fr?main_list_end_boardid=<%=endnum + 10%>">Next</a>
						</td>
						<%
				...
				<%}else if(request.getParameter("board_search_type").equals("name")){ %>
						<%
							if (!(boardsize < 10)) {
							%>
						<a href="FreeBoardSearch.fr?main_list_end_boardid=<%=startnum - 1%>&board_search_type=name&board_search_content=<%=request.getParameter("board_search_content")%>">Prev</a>
						<%
							}
						%>
						&nbsp;&nbsp;
						<a href="FreeBoardSearch.fr?main_list_end_boardid=<%=endnum + 10%>&board_search_type=name&board_search_content=<%=request.getParameter("board_search_content")%>">Next</a>
						</td>
						<%
							if (session.getAttribute("id") != null) {
						%>
					...
					<%}else if(request.getParameter("board_search_type").equals("title")){ %>
						<%
							if (!(boardsize < 10)) {
							%>
						<a href="FreeBoardSearch.fr?main_list_end_boardid=<%=startnum - 1%>&board_search_type=title&board_search_content=<%=request.getParameter("board_search_content")%>">Prev</a>
						<%
							}
						%>
						&nbsp;&nbsp;
						<a href="FreeBoardSearch.fr?main_list_end_boardid=<%=endnum + 10%>&board_search_type=title&board_search_content=<%=request.getParameter("board_search_content")%>">Next</a>
						</td>
